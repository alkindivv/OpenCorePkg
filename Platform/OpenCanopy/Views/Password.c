  ...
  )
{
  ...
  StartOffset = (mPasswordEnter.CurrentImage->Height / 2) * mPasswordEnter.CurrentImage->Width;
  ASSERT (mPasswordEnter.CurrentImage->Height > 0);
  for (
    Offset = StartOffset;
    Offset < StartOffset + mPasswordEnter.CurrentImage->Width;
    ++Offset
    )
  {
    if (mPasswordEnter.CurrentImage->Buffer[StartOffset].Reserved == 0) {
      break;
    }
  }*/
